...
document.querySelectorAll('.nav-link').forEach(link => {
  link.addEventListener('click', function(e) {
    ...
    document.querySelectorAll('main > section, section.section').forEach(s => s.style.display = 'none');
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.querySelector('#conocenos').style.display='';
  document.querySelector('#testimonios').style.display='';
});
...
function renderCarousel() {
  ...
  document.getElementById('carouselPrev').onclick = function() {
    ci = (ci <= 0) ? carouselData.length - 1 : ci - 1; renderCarousel(); renderDots();
  };
  document.getElementById('carouselNext').onclick = function() {
    ci = (ci >= carouselData.length - 1) ? 0 : ci + 1; renderCarousel(); renderDots();
  };
}
function renderDots() {
  ...
  carouselData.forEach((_, idx) => {
    ...
    dot.className = 'carousel-dot' + (idx === ci ? ' active' : '');
    dot.onclick = () => { ci = idx; renderCarousel(); renderDots(); };
    dotsEl.appendChild(dot);
  });
}
window.addEventListener('DOMContentLoaded', ()=>{ renderCarousel(); renderDots(); });
